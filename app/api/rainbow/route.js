...
export async function GET(req) {
  ...
  let hotCount = 0;
  const sums = new Array(OFFSETS.length).fill(0); // per-offset probability totals
  let inRadiusCount = 0;
  ...
  for (const c of allCells) {
    ...
    if (inRadius) {
      ...
      for (let k = 0; k < sp.series.length; k++) sums[k] += sp.series[k];
      inRadiusCount++;
      detail.push({
        lat: +c.lat.toFixed(4),
        lon: +c.lon.toFixed(4),
  ...
  // Average probability across the whole searched area at each hour offset.
  const timeline = sums.map((s) => (inRadiusCount ? +(s / inRadiusCount).toFixed(4) : 0));
