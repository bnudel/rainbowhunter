...
import { colorForProb } from "../lib/colors";
...
// A statewide-average probability of ~this much reads as a strong hour (full
// color). Averages are naturally small since most cells are 0, so this sets the
// absolute scale rather than normalizing per-request (which would make every day
// look equally good). Tune to taste.
const TRACK_REF = 0.06;

// Build a left-to-right CSS gradient for the slider track from the per-hour
// statewide averages, so the track colors show when a bow is likely anywhere.
function buildTrackGradient(timeline) {
  if (!timeline || timeline.length < 2) return null;
  const stops = timeline.map((v, i) => {
    const norm = Math.max(0, Math.min(1, v / TRACK_REF));
    const [r, g, b] = colorForProb(norm);
    const alpha = (0.18 + 0.82 * norm).toFixed(2);
    const pos = ((i / (timeline.length - 1)) * 100).toFixed(1);
...
export default function Page() {
  ...
  const trackGradient = useMemo(() => buildTrackGradient(data?.timeline), [data]);
    ...
    <>
      ...
      <div className="panel bottom-bar">
        ...
        <input
          ...
          style={trackGradient ? { "--track": trackGradient } : undefined}
        />
        <div className="ticks">
          <span>-8h</span>
          <span>-4h</span>
        ...
        <div className="slider-note">Track color = average chance across the area</div>
      </div>
